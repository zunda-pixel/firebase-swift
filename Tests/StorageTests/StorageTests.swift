...
struct StorageTests {
  let bucket: String
  let storage: StorageClient<URLSession>

  init() {
    bucket = ProcessInfo.processInfo.environment["FIREBASE_STORAGE_BUCKET"]!
    storage = StorageClient(httpClient: URLSession.shared)
  }
